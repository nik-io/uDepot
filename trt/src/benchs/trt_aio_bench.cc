...
CoroTask t_main_detached(void *arg__) {
    ...
    while (nops_submitted < nops_total) {
        ...
        for (size_t i = 0; i < nops_batch; i++) {
            co_await T::spawn(t_io, arg, nullptr, true);
            nops_submitted++;
        }
    }
...
CoroTask t_init(void *arg__)
{
    ...
    trt_dmsg("spawning aio_poller\n");
    T::spawn_detached_no_wait(AIO::poller_task, nullptr, TaskType::TASK);
