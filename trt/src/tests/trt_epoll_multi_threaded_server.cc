...
serve_task(void *arg)
{
    ...
    char indata[1];
    trt_msg("receiving ....\n");
    r_ret = co_await trt::Epoll::recv(fd, indata, sizeof(indata), 0);
    if (r_ret < 0) {
        perror("recv");
        abort();
    }
    ...
    s_ret = co_await trt::Epoll::send(fd, &handle, sizeof(handle), 0);
    if (s_ret < 0) {
        perror("send");
        abort();
    } else if (s_ret != (ssize_t)sizeof(handle)) {
...
srv_listen(void *arg)
{
    ...
    trt::Epoll::init();
    trt::T::spawn_detached_no_wait(trt::Epoll::poller_task, nullptr, trt::TaskType::TASK);

    err = url_parse(&url, url_str);
    if (err) { fprintf(stderr, "url_parse failed\n"); exit(1); }

    bool bind_ok = false;
    ai_list = url_getaddrinfo(&url, true);
    for (ai = ai_list; ai != NULL; ai = ai->ai_next) {
        fd = socket(ai->ai_family, ai->ai_socktype, ai->ai_protocol);
        if (fd == -1) { perror("socket call failed\n"); continue; }

        err = setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));
        if (err) { perror("setsockopt"); close(fd); continue; }

        if (bind(fd, ai->ai_addr, ai->ai_addrlen) < 0) {
            perror("bind"); close(fd); continue;
        }
    ...
    if (err == -1) { perror("listen failed"); exit(1); }

    struct sockaddr cli_addr;
    socklen_t cli_addr_len;
    while (true) {
        trt_msg("accept\n");
        int afd = co_await trt::Epoll::accept_ll(fd, &cli_addr, &cli_addr_len);
        trt_msg("accept returned: %d\n", afd);
        if (afd == -1) perror("accept");

        trt::TaskFn    fn   = serve_task;
        trt::TaskFnArg targ = (void *)(uintptr_t)afd;
        trt_msg("registering and spawning task...\n");
...
void client(const char url_str[])
{
    ...
    for (size_t i = 0; i < NLOOPS; i++) {
        ...
        if (fd < 0) { fprintf(stderr, "ai_connect failed\n"); exit(1); }
...
static void disable_signals(sigset_t *sigset) {
...
static void enable_signals(sigset_t *sigset) {
    int err = pthread_sigmask(SIG_UNBLOCK, sigset, NULL);
    if (err) abort();
}

static trt::CoroTask task_exit(void *arg)
{
    trt_msg("The beginning of the end ...\n");
    trt::Epoll::stop();
    co_return 0;
}

static void spawn_exit_task(trt::Scheduler &s) {
    trt::Task *t = trt::T::alloc_task(task_exit, nullptr);
    if (!t) { printf("Unable to allocate task. Cannot exit\n"); return; }
...
static void signal_exit_handler(int signum) {
    ...
    if (called_once) exit(1);
    called_once = true;
    if (!Controller_g_) exit(1);
...
int main(int argc, char *argv[])
{
    ...
    p = waitpid(p, NULL, 0);
    printf("Client DONE. Press Ctrl-C to exit\n");
